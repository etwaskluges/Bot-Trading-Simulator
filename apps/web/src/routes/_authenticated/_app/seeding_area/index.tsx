...
const seedDatabase = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        ...
        `));
            ...
            throw new Error(error.message || "Database seeding failed")
        }
    })
...
function SeedPage() {
    ...
    const [bots, setBots] = useState<{ id: string, name: string, strategy: string, balanceCents: number }[]>([])
    const [isAdvancedOpen, setIsAdvancedOpen] = useState(false)

    const [stocks, setStocks] = useState([
        { symbol: 'VIBE', name: 'Vibe Coding Inc.', price: 10000 },
        { symbol: 'TECH', name: 'Tech Innovations Corp.', price: 5000 },
        { symbol: 'GROW', name: 'Growth Enterprises Ltd.', price: 7500 },
    ])
    ...
    const handleUpdateBot = (id: string, field: string, value: any) => {
        setSyncQuickConfig(false)
        setBots(bots.map(b => b.id === id ? { ...b, [field]: value } : b))
    }

    const handleAddStock = () => {
        setStocks([...stocks, { symbol: 'NEW', name: 'New Company', price: 1000 }])
    }
    ...
    const handleUpdateStock = (index: number, field: string, value: any) => {
                    ...
                    <div className="space-y-6">
                        <div className="bg-card p-6 rounded-2xl border shadow-lg flex flex-col h-[500px]">
                            ...
                            <div className="flex-1 space-y-6 overflow-y-auto overflow-x-hidden pr-2 scrollbar-thin scrollbar-thumb-muted">
                                <div className="space-y-4">
                                    <div className="grid grid-cols-3 gap-3">
                                        {(Object.keys(strategyCounts) as Array<keyof typeof strategyCounts>).map((strat) => (
                                            <div key={strat}>
                                                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.12em] mb-1.5 block">{strat}</label>
                                                <input
                                                    type="number"
                                                    value={strategyCounts[strat]}
                                                    onChange={(e) => setStrategyCounts({ ...strategyCounts, [strat]: Number(e.target.value) })}
                                    ...
                                    <div>
                                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.12em] mb-1.5 block">Default Bot Balance ($)</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.12em] mb-1.5 block">Min Initial Shares</label>
                                            <input
                                                type="number"
                                                value={sharesMin}
                                                onChange={(e) => setSharesMin(Number(e.target.value))}
                                        ...
                                        <div>
                                            <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.12em] mb-1.5 block">Max Initial Shares</label>
                                            <input
                                                type="number"
                                                value={sharesMax}
                                                onChange={(e) => setSharesMax(Number(e.target.value))}
                                                className="w-full bg-muted/30 border border-border/50 rounded-xl px-4 py-2.5 font-mono font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm"
                                            />
                                        </div>
                                    </div>
                                ...
                                <div className="pt-4 border-t border-dashed border-muted-foreground/10 space-y-4">
                                    <label className="flex items-center gap-3 cursor-pointer group">
                                        <div className="relative">
                                            <input
                                                type="checkbox"
                                                checked={syncQuickConfig}
                                                onChange={(e) => setSyncQuickConfig(e.target.checked)}
                                    ...
                                    {/* INTEGRATED MANUAL BOT CONFIG - FULL WIDTH */}
                                    <div className="-mx-6 border-t border-border/50 bg-muted/20">
                                        ...
                                        >
                                            <div className="flex items-center gap-2.5">
                                                <div className={`p-1.5 rounded-lg transition-colors ${isAdvancedOpen ? 'bg-primary/20 text-primary' : 'bg-primary/10 text-primary'}`}>
                                                    <Bot size={16} />
                                                </div>
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.15em] whitespace-nowrap">
                                                    Manual Bot Config
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {isAdvancedOpen ? <ChevronUp size={14} className="text-muted-foreground" /> : <ChevronDown size={14} className="text-muted-foreground" />}
                                            </div>
                                        </button>

                                        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isAdvancedOpen ? 'max-h-[1000px] opacity-100 px-6 pb-4 pt-0' : 'max-h-0 opacity-0'}`}>
                                            <div className="space-y-3 pt-4 border-t border-border/30">
                                                ...
                                                <div className="space-y-2">
                                                    {bots.map((bot) => (
                                                        <div key={bot.id} className="p-4 rounded-2xl border border-border/50 bg-background/50 relative group flex flex-col gap-3">
                                                            <button
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3">
                                                                <div className="col-span-2">
                                                                    <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-widest mb-1 block">Name</label>
                                                                    <input
                                                                        value={bot.name}
                                                                        onChange={(e) => handleUpdateBot(bot.id, 'name', e.target.value)}
                                                                ...
                                                                <div>
                                                                    <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-widest mb-1 block">Strategy</label>
                                                                    <select
                                                                        value={bot.strategy}
                                                                        onChange={(e) => handleUpdateBot(bot.id, 'strategy', e.target.value)}
                                                                ...
                                                                <div>
                                                                    <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-widest mb-1 block">Cash ($)</label>
                                                                    <div className="relative">
                                                                        <input
                                                                            type="number"
                                                                            value={bot.balanceCents / 100}
                                                                            onChange={(e) => handleUpdateBot(bot.id, 'balanceCents', Number(e.target.value) * 100)}
                    ...
                    <div className="bg-card p-6 rounded-2xl border shadow-lg flex flex-col h-[500px]">
                        ...
                        <div className="flex-1 space-y-3 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-muted">
                            {stocks.map((stock, idx) => (
                                <div key={idx} className="p-4 rounded-2xl border bg-muted/30 hover:bg-muted/50 transition-all flex flex-col gap-3 relative group">
                                    <button
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        <div className="col-span-1 text-center">
                                            <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracing-widest mb-1 block">Ticker</label>
                                            <input
                                                value={stock.symbol}
                                                onChange={(e) => handleUpdateStock(idx, 'symbol', e.target.value)}
                                        ...
                                        <div className="col-span-2">
                                            <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracing-widest mb-1 block">Company Name</label>
                                            <input
                                                value={stock.name}
                                                onChange={(e) => handleUpdateStock(idx, 'name', e.target.value)}
                                                className="w-full bg-background border border-border/50 rounded-lg px-3 py-1.5 text-xs font-semibold focus:ring-2 focus:ring-primary/10 outline-none transition-all"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <label className="text-[9px] font-bold text-muted-foreground/50 uppercase tracing-widest mb-1 block">Initial Price (cents)</label>
                                        <div className="relative">
                                            <input
                                                type="number"
                                                value={stock.price}
                                                onChange={(e) => handleUpdateStock(idx, 'price', Number(e.target.value))}
                                                className="w-full bg-background border border-border/50 rounded-lg pl-8 pr-3 py-1.5 text-xs font-mono font-bold focus:ring-2 focus:ring-primary/10 outline-none transition-all"
                                            />
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        {status && (
                            <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                                <div className={`px-5 py-2.5 rounded-xl border flex items-center gap-2.5 shadow-sm ${status.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' : 'bg-red-500/10 border-red-500/20 text-red-600'
                                    }`}>
