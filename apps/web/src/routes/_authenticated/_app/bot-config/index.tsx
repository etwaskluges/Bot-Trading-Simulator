...
import { Bot, ChevronDown, ChevronUp, Plus, Users, X } from 'lucide-react'
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-[0.12em] text-primary hover:bg-primary/10 transition-colors"
        ...
        <div className="space-y-2">
          ...
          {quickStrategies.map((quickStrategy) => (
            <div key={quickStrategy.id} className="grid grid-cols-[1fr_80px_auto] gap-2 items-center">
              <select
...
function BotConfigPanel({
  ...
  return (
    <div className="space-y-6">
      <div className="bg-card p-6 rounded-2xl border shadow-lg flex flex-col h-[640px]">
...
function BotConfigPage() {
  ...
  const loadStrategies = async () => {
    ...
    try {
      const data = await fetchUserStrategies()
      setStrategies(data)
  ...
  const handleStartBots = async () => {
    ...
    try {
      const session = await startUserBots()
  ...
  const handleStopBots = async () => {
    ...
    try {
      ...
      const stoppedCount = Array.isArray(result.stopped) ? result.stopped.length : 0
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 flex flex-wrap items-center justify-center gap-3">
          <button
            ...
            className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-[0.15em] bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm"
          >
            {isCreatingBots ? 'Creating Bots...' : 'Create Bots'}
          </button>
          <button
            type="button"
            onClick={handleStartBots}
            disabled={isStartingBots}
            className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-[0.15em] bg-emerald-500 text-white hover:bg-emerald-500/90 transition-colors shadow-sm disabled:opacity-60"
          >
            {isStartingBots ? 'Starting Bots...' : 'Start Bots'}
          </button>
          <button
            type="button"
            onClick={handleStopBots}
            disabled={isStoppingBots}
            className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-[0.15em] bg-red-500 text-white hover:bg-red-500/90 transition-colors shadow-sm disabled:opacity-60"
          >
            {isStoppingBots ? 'Stopping Bots...' : 'Stop Bots'}
          </button>
        </div>
        {createStatus && (
          <div className={`mb-6 text-center text-xs font-bold ${createStatus.type === 'success' ? 'text-emerald-600' : 'text-red-600'}`}>
            {createStatus.message}
          </div>
        )}
        {botControlStatus && (
          <div className={`mb-6 text-center text-xs font-bold ${botControlStatus.type === 'success' ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        <BotConfigPanel
          ...
          onReloadStrategies={loadStrategies}
...
export const Route = createFileRoute('/_authenticated/_app/bot-config/')({
